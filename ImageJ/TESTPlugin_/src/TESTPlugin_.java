...
public class TESTPlugin_ implements PlugInFilter, DialogListener {
	...
	public void run(ImageProcessor ip) {
		...
		int tailleMask = (int) (7 * sigma);
		// Passage à taille impaire
		if (tailleMask % 2 == 0) {
			tailleMask++;
		}

		final float[] mask = masqueLoG(tailleMask, sigma);
		
		// Calcul et représentation du Laplacien (exercice 1, puis à modifier
		// dans le 3)
		
		final FloatProcessor fpLaplacian = (FloatProcessor) (ip.duplicate().convertToFloat());
		if (sigma != 0) {
			final Convolver conv = new Convolver();
			conv.setNormalize(false);
			conv.convolve(fpLaplacian, mask, tailleMask, tailleMask);
		} else {
			fpLaplacian.convolve(MASQUES_LAPLACIENS3x3[filtre], 3, 3);
		}
		
		// Afficher l’image si possible
		final ImagePlus imgPlacian = new ImagePlus(titre + "_convolveLaplacien", fpLaplacian);
		imgPlacian.show();
		
		// D�tection et affichage des passages par 0 du Laplacien par seuillage
		// du Laplacien (exercice 2)
		if (seuillageZeroCross) {
			final ImagePlus imgSeuil = new ImagePlus(titre + "_seuilLaplacien" +
					String.valueOf(seuilZeroCross), laplacienZero(fpLaplacian, seuilZeroCross));
			imgSeuil.show();
		}
	}
	 ...
	 * 
	 * @return false si la fen�tre a �t� ferm�e en cliquant sur Cancel, true
	 *         sinon (boolean)
	 */
	...
	public boolean showDialog() {
		...
		gd.getComponent(gd.getComponentCount() - 1).setEnabled(seuillageZeroCross && seuilZeroCrossAuto); // D�sactiver
																											// case
																											// ?
		gd.addDialogListener(this); // the DialogItemChanged method will be
									// called on user input
		gd.showDialog(); // display the dialog; preview runs in the background
							// now
		if (gd.wasCanceled())
			return false;
	...
	// ---------------------------------------------------------------------------------
	/**
	 * M�thode appel�e sur modification (par un �v�nement) de la fen�tre de
	 * config
	 * 
	 * @param gd
	 *            Fen�tre de dialogue (GenericDialog)
	 * @param e
	 *            �v�nement � traiter (AWTEvent)
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (e != null) { // e==null si clic sur OK
			// D�sactivation/activation du champ num�rique de seuil, selon
			// seuillage ou pas
	 ...
	 * 
	 * @param imLaplacien
	 *            Image du Laplacien (ImageProcessor 32 bits)
	 * @param seuil
	 *            Seuil sur les valeurs du Laplacien
	...
	public ByteProcessor laplacienZero(ImageProcessor imLaplacien, Float seuil) {
		...
		final int width = imLaplacien.getWidth();
		final int height = imLaplacien.getHeight();
		...
		for (int y = 1; y < height - 1; y++) {
			for (int x = 1; x < width - 1; x++) {
				...
				// Parcours d’un voisinage 3x3
				for (int j = -1; j < 2; j++) {
					for (int i = -1; i < 2; i++) {
						final float value = imLaplacien.getf(x + i, y + j);
						if (min > value) {
							min = value;
						} else if (max < value) {
							max = value;
				...
				imZeros.set(x, y, min < -seuil && max > seuil
								? 255
								: 0);
			}
		}
	 ...
	 * 
	 * @param tailleMasque
	 *            nombre de lignes et de colonnes du masque LoG (int, impair).
	 * @param sigma
	 *            �cart-type de la gaussienne (float, au moins 5*sigma)
